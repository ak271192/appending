...
#file name with logging type mode
...
#maskPath1 = "C:/Users/Dpotti/Desktop/Jeans/jeansalt.png"
    maskPath1 = "C:/Users/Dpotti/Desktop/pantalt.png"
...
##gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    args.input = "C:/Users/Dpotti/Pictures/Result/Person/image.jpg"
    inWidth = args.width
    ...
    shirt = "C:/Users/Dpotti/Desktop/Datasets/fullhand.png"
    person = "C:/Users/Dpotti/Pictures/Result/Person/image.jpg"

    pant = "C:/Users/Dpotti/Desktop/pantalt.png"
    shirtopen = Image.open(shirt).convert("RGBA")
    Personopen = Image.open(person).convert("RGBA")
    Pantopen = Image.open(pant).convert("RGBA")
    Personopen.show()
    shirtopen.show()
    Pantopen.show()
...
#            cv.ellipse(frame, points[idTo], (3, 3), 0, 0, 360, (0, 0, 2    55), cv.FILLED)
...
#    background.show()
  #############################################################
        #foreground = Image.open("C:/Users/Dpotti/Desktop/shirt1.png")
    #width,height
        w = int(w*1.74) # For 800 pixel image size of human picture
        h = int(h*1.3)
   ...
   # mask = Image.open(maskPath).convert("RGBA")
        ...
        if points[8][1] == points[11][1]:
            ...
            print(points)
            print("entered inside else cond")
            L1 = list(points[11])
            print(L1)
            L1[1] = points[8][1]
            points[11] = tuple(L1)
            
            print("after tuple conv",points)
            print(points[8])
            print(points[11])
        if points[8][0] == points[10][0]:
            print("length proceed")
        else:
            print(points)
            print("entered inside else cond")
            L1 = list(points[10])
        ...
        lhip = points[11]
        print("lhip", lhip)
        bsub = tuple(map(sub, lhip, rhip))
        print(bsub)
        rhip = points[8]
        print("rhip", rhip)
        rankle = points[10]
        print("rankle", rankle)
        l = tuple(map(sub, rhip, rankle))
        print(l)
        bsub,l = tuple(map(sub, bsub, l))
        print(bsub,l, "detect")
    
    
    
        bsub = int(bsub*2) # For 800 pixel image size of human picture
        l = int(l*1.3)
        
        resized_mask1 = mask1.resize((bsub,l), Image.ANTIALIAS)
        a1 = points[8]
        b1 = (43,30)
        offset1 = tuple(map(sub, a1, b1))
        print(offset1)
        #offset = points[2]
   # mask = Image.open(maskPath).convert("RGBA")
...
#    resized_mask1 = mask1.resize((380,440), Image.ANTIALIAS)
#    a = points[8]
#    #move right side to increase, 
#    b = (133,40)
...
##    cv.imshow('Live', background)
#    background.show()
   # background.show("C:/Users/Dpotti/Pictures/Result/Apparel/Datasets/Png/pant5.png")
